...
buttons.forEach((button) => {
  ...
  const confirmTextElements = button.querySelectorAll(
    ...
    words.forEach((word, index) => {
      if (word === "sure?") {
        ...
        characters.forEach((char, index2) => {
          letterHTML += `<span class="char" style="--char-delay:${
            index2 * 100 + (index * 300 + 450)
          }ms;">${char}</span>`;
        });
        wordHTML += `<span class="word word--bold" style="--word-delay:${
          index * 300 + 400
        }ms;">${letterHTML}</span>`;
      } else {
        wordHTML += `<span class="word" style="--word-delay:${
          index * 300 + 400
        }ms;">${word}</span>`;
      }
    });
